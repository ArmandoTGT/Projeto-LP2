...
public class LSE {
	...
	public void remove(int pos) {
	    ...
	    if (pos == 1){
			System.out.println("\nRemovida da posição inicial");
	    	removeInicioLista();
	    	return;
	    }
	    // RemoÃ§Ã£o em outro lugar da lista
	    else{
			System.out.println("\nRemovida da posição " + pos);
	    	removeNaLista(pos);
	        return;
	    }
	}

	//Recebemos a referência para a sala
	public void salaVazia(NoBuffer verifica){
		
		
		System.out.println("\nSala achada para esvaziar:" + verifica);
		//Agora verificamos se aquele NoBuffer está vazio, se sim acontece a remoção, se não segue o jogo
		if(verifica.canalVazio()) {
			System.out.println("\nEsvaziada");
			remove(canal(verifica) + 1); //Remove o número daquele canal atualmente registrado no servidor
		}
		else {
			return;//Se não o método como é void, não faz nada
		}
	}
	...
	/*
	 * Vamos procurar em todos os nós a partir de uma fila de nós começando em 0
	 * até achar uma sala com uma posição vazia e retornará o ponteiro dessa sala
	 */
	...
	//Primeiro pegamos a referência desse canal
	public int canal(NoBuffer canal){
		NoBuffer aux = cabeca;
		int nCanal = 0;
		
		//Então vamos subindo um contador até chegar no canal
		while(aux != canal){
			if(aux == null) return -1; //Caso não achemos a sala passada, vamos retonar um valor inválido(apenas para mais robustes)
			nCanal++;
			aux = aux.getProx();
		}
