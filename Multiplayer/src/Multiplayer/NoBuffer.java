...
public class NoBuffer {
	...
	private  String mensagem[];//Cada posição se trata de um buffer próprio onde o jogador escreve seus dados
	private boolean online[]; //Estado se o jogador está online ou não na sala
	//Essas variaveis serão úteis na realocação de clientes
	private int espacos;
	...
	//Essa função vai verificar se alguma sala(ponteiro) possui alguma posição vazia
	public boolean posVazia() {
		/*
	...
	//Essa função vai olhar se não ficou nenhum jogador online na sala
	public boolean canalVazio(){
		//Se em algum momento nodentre as posições do array, ele achar uma posição online, a sala não está vazia
		for(int i = 0; i < online.length; i++) {
			System.out.println("Canal Vazio" + online[i]);
			if(online[i] == true) { 
				return false;
			}
		}
	...
	/*
	* Método que escreve a mensagem que será mandada, ela recebe como parametros a mensagem e o id,
	* pois o a mensagem será guardada apenas no "buffer simples" ligado ao identificador do player
	*/
	...
	//Método leitor que os dados referentes a cada jogador
